...
namespace Perspex.Markup.UnitTests.Binding
{
    public class ExpressionNodeBuilderTests
    {
        ...
        {
            var result = ToList(ExpressionNodeBuilder.Build("Foo[5]"));

            Assert.Equal(2, result.Count);
            AssertIsProperty(result[0], "Foo");
            AssertIsIndexer(result[1], 5);
            Assert.IsType<ElementAccessorNode>(result[1]);
            Assert.Equal(new object[] { 5 }, ((ElementAccessorNode)result[1]).Arguments.ToArray());
        }

        [Fact]
        public void Should_Build_Multiple_Indexed_Property()
        {
            var result = ToList(ExpressionNodeBuilder.Build("Foo[5, 6]"));

            Assert.Equal(2, result.Count);
            AssertIsProperty(result[0], "Foo");
            Assert.IsType<ElementAccessorNode>(result[1]);
            Assert.Equal(new object[] { 5, 6 }, ((ElementAccessorNode)result[1]).Arguments.ToArray());
        }
