...
namespace Avalonia.Skia.UnitTests.Media.TextFormatting
{
    ...
    /// <c>ResetTrailingWhitespaceBidiLevels</c>). These exist to pin the
    /// current observable behaviour before the wrap-path optimisation work
    /// in <c>docs/perform-text-wrapping-plan.md</c>. They go through the
    /// public <c>TextFormatterImpl.FormatLine</c> entry point so the helpers
    /// can remain private.
    /// </summary>
    public class TextFormatterWrapCharacterizationTests
    {
        // --- MeasureLength behaviour, observed via FormatLine -----------------
        ...
        // --- Overflow + lookahead path (lines 962-981 of TextFormatterImpl) ---
        // This is the smell flagged as D1 in the optimisation plan: the
        // currentPosition += accumulation looks wrong because PositionWrap is
        // absolute within the run, not delta. These tests pin what actually
        // happens today so we can detect any behavioural change while
        // refactoring.
        ...
        // --- Continued wrap (WrappingTextLineBreak path) ----------------------
