...
namespace Avalonia.Controls.UnitTests
{
    public class VirtualizingStackPanelTests : ScopedTestBase
    {
        ...
        /// <summary>
        /// The horizontal counterpart of <see cref="UnroundedCanvasWithHeightTemplate"/>.
        /// </summary>
        private static FuncDataTemplate<ItemWithWidth> UnroundedCanvasWithWidthTemplate = new((_, _) =>
            new CanvasCountingMeasureArrangeCalls
            {
                Height = 100,
                UseLayoutRounding = false,
        ...
        // ===== Category: the size record's index remapping =====
        //
        // Run in both orientations: the record stores a size along the scrolling axis and knows
        // nothing about which axis that is, so every contract below has to hold identically for a
        // horizontal panel. A per-orientation branch creeping into the record (or into the extent
        // arithmetic that reads it) shows up here rather than in a bug report.

        [Theory]
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Measured_Size_Record_Shifts_Up_On_Insert_At_The_Front(Orientation orientation)
        {
            using var app = App();
            var (target, scroll, items) = CreateTargetWithPopulatedSizeRecord(orientation);
            ...
            items.InsertRange(0, new[] { CreateItemU(orientation, 900, 33), CreateItemU(orientation, 901, 34) });

            AssertMeasuredSizeRecordEquals(expected, target, items.Count + 4);

            Layout(target);
            AssertExtentAgreesWithMeasuredSizeRecord(target, scroll, items.Count, orientation);
        }

        [Theory]
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Measured_Size_Record_Shifts_Up_On_Mid_List_Insert(Orientation orientation)
        {
            using var app = App();
            var (target, scroll, items) = CreateTargetWithPopulatedSizeRecord(orientation);
            ...
            items.InsertRange(20, Enumerable.Range(0, 3).Select(i => CreateItemU(orientation, 900 + i, 35 + i)));
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Measured_Size_Record_Is_Untouched_By_An_Insert_Past_The_Last_Item(Orientation orientation)
        {
            using var app = App();
            var (target, scroll, items) = CreateTargetWithPopulatedSizeRecord(orientation);
            ...
            items.InsertRange(items.Count, Enumerable.Range(0, 3).Select(i => CreateItemU(orientation, 900 + i, 35 + i)));
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Measured_Size_Record_Shifts_Down_On_Remove_At_The_Front(Orientation orientation)
        {
            using var app = App();
            var (target, scroll, items) = CreateTargetWithPopulatedSizeRecord(orientation);
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Measured_Size_Record_Shifts_Down_On_Mid_List_Remove(Orientation orientation)
        {
            using var app = App();
            var (target, scroll, items) = CreateTargetWithPopulatedSizeRecord(orientation);
        ...
        /// </summary>
        private static (VirtualizingStackPanel, ScrollViewer, AvaloniaList<object>) CreateTargetWithPopulatedSizeRecord(
            Orientation orientation)
        {
            var items = new AvaloniaList<object>(CreateItemsU(orientation, 40, x => 20 + x));
            var (target, scroll, _) = CreateTarget(
                items: items,
                itemTemplate: ItemTemplateU(orientation),
                orientation: orientation,
                bufferFactor: 0d);
            ...
            {
                scroll.Offset = OffsetU(orientation, offset);
                Layout(target);
            }
            ...
            Orientation orientation)
        {
            ...
            var extentU = ExtentU(orientation, scroll);
            Assert.True(Math.Abs(expected - extentU) <= 1d,
                $"Extent {extentU} does not match the {record.Count} recorded sizes " +
                $"(sum {knownSum}, mean {mean}, {itemCount} items => {expected}).");
        }

        // ===== Category: what the size record costs in memory =====
        //
        // The record is deliberately never trimmed and never re-summed: that is what makes the
        // extent reproducible when an offset is revisited (see the estimate tests above). These
        // tests pin the price of that decision so it is a documented figure rather than something
        // a reviewer discovers - one entry per item ever measured, released only when the
        // index-to-item mapping stops being trustworthy.

        [Theory]
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Measured_Size_Record_Holds_One_Entry_Per_Item_Ever_Measured(Orientation orientation)
        {
            using var app = App();

            var items = new AvaloniaList<object>(CreateItemsU(orientation, 40, _ => 20));
            var (target, scroll, _) = CreateTarget(
                items: items,
                itemTemplate: ItemTemplateU(orientation),
                orientation: orientation,
            ...
            // Only the realized window has been measured so far, so the record is that small.
            var realized = target.GetRealizedElements().Count();
            Assert.Equal(realized, CountMeasuredSizeEntries(target, items.Count + 4));

            // Browse the whole collection: one entry appears per item, and no entry appears for
            // an index that does not exist.
            BrowseToEnd(target, scroll, step: 100d, end: 700d, orientation);

            Assert.Equal(40, CountMeasuredSizeEntries(target, items.Count + 4));
            Assert.Equal(
                Enumerable.Range(0, 40),
                ReadMeasuredSizeRecord(target, items.Count + 4).Keys.OrderBy(k => k));

            // Browsing again re-measures the same items; entries are upserted, not appended, so
            // the record does not keep growing with time spent scrolling.
            BrowseToEnd(target, scroll, step: 100d, end: 700d, orientation);
            Assert.Equal(40, CountMeasuredSizeEntries(target, items.Count + 4));
        }
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Measured_Size_Record_Is_Not_Trimmed_When_Items_Leave_The_Viewport(Orientation orientation)
        {
            using var app = App();
            ...
            BrowseToEnd(target, scroll, step: 100d, end: 1500d, orientation);
            var extentAtEnd = ExtentU(orientation, scroll);

            scroll.Offset = OffsetU(orientation, 0);
            Layout(target);

            // Back at the top only a handful of items are realized, but every item measured on
            // the way down still has its entry ...
            Assert.Equal(40, CountMeasuredSizeEntries(target, items.Count + 4));
            var realizedIndices = target.GetRealizedElements()
                .Select((_, i) => target.FirstRealizedIndex + i)
                .ToHashSet();
            Assert.Contains(
                ReadMeasuredSizeRecord(target, items.Count + 4).Keys,
                k => !realizedIndices.Contains(k));

            // ... which is the point of not trimming: the extent is the same number as it was at
            // the far end of the collection, not an estimate re-derived from the realized window.
            Assert.Equal(extentAtEnd, ExtentU(orientation, scroll));
        }

        [Theory]
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Measured_Size_Record_Is_Dropped_On_Reset(Orientation orientation)
        {
            using var app = App();
            ...
            // A Reset makes every index-to-item mapping untrustworthy, so the whole record goes -
            // the one point at which the accumulated memory is handed back.
            items.Clear();
            Layout(target);

            Assert.Equal(0, CountMeasuredSizeEntries(target, 44));
        }

        [Theory]
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Measured_Size_Record_Is_Dropped_On_Move(Orientation orientation)
        {
            using var app = App();
            ...
            // A move shifts an arbitrary range; rather than track the permutation the record is
            // cleared, so no entry can end up describing another item.
            items.Move(2, 30);

            Assert.Equal(0, CountMeasuredSizeEntries(target, items.Count + 4));
        }

        [Theory]
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Measured_Size_Record_Survives_Detach_But_Not_A_Re_Attach(Orientation orientation)
        {
            using var app = App();
            ...
            // Detaching does not invalidate the index-to-item mapping, so nothing is dropped -
            // which also means the memory is held for as long as the panel is alive.
            var root = Assert.IsType<TestRoot>(itemsControl.Parent);
            root.Child = null;

            Assert.Equal(40, CountMeasuredSizeEntries(target, items.Count + 4));

            // Re-attaching, however, re-registers the items and the panel handles that as a Reset,
            // which does clear the record: the extent is rebuilt from the realized window, exactly
            // as on a first layout. So the record survives the detached state, not a round trip
            // through the tree.
            root.Child = itemsControl;
            Layout(target);

            Assert.Equal(
                target.GetRealizedElements().Count(),
                CountMeasuredSizeEntries(target, items.Count + 4));
        }

        /// <summary>
        /// Scrolls from wherever the panel is to <paramref name="end"/> in <paramref name="step"/>
        /// increments, laying out at each one, so every item in between is realized and measured.
        /// </summary>
        private static void BrowseToEnd(
            VirtualizingStackPanel target, ScrollViewer scroll, double step, double end, Orientation orientation)
        {
            for (var offset = 0d; offset <= end; offset += step)
            {
                scroll.Offset = OffsetU(orientation, offset);
                Layout(target);
            }
        ...
        private static int CountMeasuredSizeEntries(VirtualizingStackPanel target, int scanTo)
        {
            var count = 0;

            for (var i = 0; i <= scanTo; ++i)
            {
                if (target.TryGetMeasuredSizeForTesting(i, out _))
                    ++count;
            }
        ...
        // ===== Orientation-agnostic accessors =====
        //
        // The panel works in (u, v) space where u is the scrolling axis, and the per-item size
        // record, the extent arithmetic and the anchor compensation are all written against u
        // rather than against Y. Tests whose contract is likewise axis-independent therefore run
        // in BOTH orientations, reading and writing u through these accessors, so a horizontal
        // regression cannot hide behind a vertically-written assertion. The test root is square
        // (100x100) and the u-templates fix the cross-axis at 100, so the two orientations see
        // the same geometry and can share the same expected numbers.

        private static readonly Orientation[] BothOrientations =
            { Orientation.Vertical, Orientation.Horizontal };

        /// <summary>A scroll offset of <paramref name="u"/> along the scrolling axis.</summary>
        private static Vector OffsetU(Orientation orientation, double u) =>
            orientation == Orientation.Horizontal ? new Vector(u, 0) : new Vector(0, u);

        private static double ScrollOffsetU(Orientation orientation, ScrollViewer scroll) =>
            orientation == Orientation.Horizontal ? scroll.Offset.X : scroll.Offset.Y;

        private static double ExtentU(Orientation orientation, ScrollViewer scroll) =>
            orientation == Orientation.Horizontal ? scroll.Extent.Width : scroll.Extent.Height;

        /// <summary>The element's leading edge along the scrolling axis.</summary>
        private static double BoundsStartU(Orientation orientation, Visual element) =>
            orientation == Orientation.Horizontal ? element.Bounds.Left : element.Bounds.Top;

        /// <summary>The element's size along the scrolling axis.</summary>
        private static double BoundsSizeU(Orientation orientation, Visual element) =>
            orientation == Orientation.Horizontal ? element.Bounds.Width : element.Bounds.Height;

        /// <summary>
        /// An item whose size along the scrolling axis is <paramref name="sizeU"/>, matched by
        /// <see cref="ItemTemplateU"/>. The cross-axis size is fixed by the template.
        /// </summary>
        private static object CreateItemU(Orientation orientation, int index, double sizeU) =>
            orientation == Orientation.Horizontal
                ? new ItemWithWidth(index, sizeU)
                : (object)new ItemWithHeight(index, sizeU);

        private static List<object> CreateItemsU(Orientation orientation, int count, Func<int, double> sizeU) =>
            Enumerable.Range(0, count).Select(i => CreateItemU(orientation, i, sizeU(i))).ToList();

        // Returned as the Optional CreateTarget takes: C# will not run the user-defined
        // conversion to Optional<IDataTemplate?> from an interface-typed expression.
        private static Optional<IDataTemplate?> ItemTemplateU(Orientation orientation) =>
            orientation == Orientation.Horizontal
                ? new Optional<IDataTemplate?>(CanvasWithWidthTemplate)
                : new Optional<IDataTemplate?>(CanvasWithHeightTemplate);

        private static Optional<IDataTemplate?> UnroundedItemTemplateU(Orientation orientation) =>
            orientation == Orientation.Horizontal
                ? new Optional<IDataTemplate?>(UnroundedCanvasWithWidthTemplate)
                : new Optional<IDataTemplate?>(UnroundedCanvasWithHeightTemplate);

        /// <summary>
        /// A container-content template whose fixed size is <paramref name="sizeU"/> along the
        /// scrolling axis and 100 across it — the orientation-agnostic form of the
        /// <c>new Canvas { Width = 100, Height = 10 }</c> the type-keyed template tests use.
        /// </summary>
        private static Canvas CanvasU(Orientation orientation, double sizeU, double sizeV = 100) =>
            orientation == Orientation.Horizontal
                ? new Canvas { Width = sizeU, Height = sizeV }
                : new Canvas { Width = sizeV, Height = sizeU };

        /// <summary>
        /// Read/write access to an item's size along the panel's scrolling axis, so a test can
        /// resize an item without knowing which orientation it is running in.
        /// </summary>
        private interface ISizedItemU
        {
            double SizeU { get; set; }
        }

        private class ItemWithHeight : NotifyingBase, ISizedItemU
        {
        ...
        private class ItemWithWidth : NotifyingBase, ISizedItemU
        {
        ...
        private class VirtualizingStackPanelWithInstability : VirtualizingStackPanel
        {
            ...
            /// counted <see cref="VirtualizingStackPanel.ElementSizeAdjustmentForTesting"/> calls,
            /// but that seam is consulted wherever
            ...
            public VirtualizingStackPanelWithInstability()
            {
                // The first size an item is seen at is provisional; it settles afterwards.
                ElementSizeAdjustmentForTesting = (index, measuredSizeU) =>
                    EnableInstability && _perturbed.Add(index)
        ...
        /// per index via the element-size seam. Because the adjustment never changes, the
        ...
        /// <see cref="VirtualizingStackPanel.ElementSizeAdjustmentForTesting"/> seam: the first
        /// time an element at a given index is measured it reports a small "placeholder" size, and
        ...
        private class VirtualizingStackPanelAsyncGrow : VirtualizingStackPanel
        {
            ...
            public VirtualizingStackPanelAsyncGrow()
            {
                ElementSizeAdjustmentForTesting = (index, _) =>
                {
        ...
        {
            using var app = App();
            ContainerVirtualization.IsEnabled = true;
        ...
        // ===== Category E: Warmup =====
        //
        // Warmup pre-creates containers off the recycle keys the panel has encountered, which is
        // bookkeeping over template keys and says nothing about geometry — so it has to behave the
        // same in a horizontal panel, and these run in both orientations to say so. Only the item
        // size the template hands out is swapped (CanvasU), so ~10 containers realize either way.

        [Theory]
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Warmup_PreCreates_Containers_For_Discovered_Keys(Orientation orientation)
        {
            using var app = App();
            ContainerVirtualization.IsEnabled = true;
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Warmup_Respects_MinPoolSizePerKey(Orientation orientation)
        {
            using var app = App();
            ContainerVirtualization.IsEnabled = true;
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Warmup_Containers_Are_Reused_On_First_Scroll(Orientation orientation)
        {
            using var app = App();
            ContainerVirtualization.IsEnabled = true;
            ...
            try
            {
                ...
                // Scroll on one page — pool containers should be consumed
                scroll.Offset = OffsetU(orientation, 100);
                Layout(target);
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Warmup_Pools_Keys_First_Encountered_Outside_The_Head_Of_The_Collection(Orientation orientation)
        {
            ...
            try
            {
                ...
                scroll.Offset = OffsetU(orientation, 2500);
                Layout(target);
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Warmup_Forgets_Template_Keys_The_Collection_No_Longer_Contains(Orientation orientation)
        {
        ...
        // ----- Warmup and the visual tree -----
        //
        // Warmup is the one piece of the panel that does work *outside* a layout pass: attaching
        // posts it to the dispatcher at Background priority, so it runs a turn later. The three
        // tests below are about the window between the post and the tick, which is where the
        // panel's lifetime and the dispatcher's disagree. They drive the posted job (RunJobs)
        // rather than calling PerformWarmup directly, because the post is the thing under test.

        [Theory]
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Warmup_Is_Posted_By_Attach_And_Runs_On_The_Dispatcher(Orientation orientation)
        {
            // Control for the two tests that follow: they assert that nothing happens on the
            // dispatcher tick, which is only meaningful if something happens here.
            using var app = App();
            ContainerVirtualization.IsEnabled = true;

            try
            {
                var (target, itemsControl, root) = CreateWarmupTarget(orientation);

                // Nothing has been recycled yet, so anything in the pool came from warmup.
                Assert.True(PooledCount(target, typeof(TypeA_Item)) == 0,
                    "Test setup: the pool should be empty until warmup runs.");

                Dispatcher.UIThread.RunJobs();

                Assert.True(PooledCount(target, typeof(TypeA_Item)) > 0,
                    "Attaching to the visual tree should have posted warmup, and running the " +
                    "dispatcher should have performed it.");
            }
            finally
            {
                ContainerVirtualization.IsEnabled = true;
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Warmup_Detached_Before_Its_Dispatcher_Tick_Neither_Throws_Nor_Builds_Containers(
            Orientation orientation)
        {
            // A page navigated away from, or a window closed, within the frame between
            // OnAttachedToVisualTree posting warmup and the dispatcher getting to it. The panel is
            // still attached to its ItemsControl at that point — only the visual tree let go — so
            // Items and the generator are both still live and warmup would happily run.
            //
            // It must not. The containers it builds would be parked in Panel.Children and the
            // recycle pool for as long as the panel is referenced, having bought nothing: warmup
            // exists to move template instantiation off the first scroll, and a panel nobody is
            // showing has no first scroll to move it off.
            using var app = App();
            ContainerVirtualization.IsEnabled = true;
            ...
            {
                var (target, itemsControl, root) = CreateWarmupTarget(orientation);
                var childrenBeforeDetach = target.Children.Count;

                root.Child = null;
                Assert.Null(target.GetVisualRoot());

                var exception = Record.Exception(() => Dispatcher.UIThread.RunJobs());

                Assert.Null(exception);
                Assert.Equal(0, PooledCount(target, typeof(TypeA_Item)));
                Assert.Equal(childrenBeforeDetach, target.Children.Count);
            }
            finally
            {
                ContainerVirtualization.IsEnabled = true;
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Warmup_Skipped_While_Detached_Runs_When_The_Panel_Is_Attached_Again(
            Orientation orientation)
        {
            // Skipping warmup while detached must be a postponement, not a cancellation: a panel
            // that is shown again — the navigated-back-to page — has to warm up then. Otherwise
            // the skip would silently turn EnableWarmup off for the rest of the panel's life.
            using var app = App();
            ContainerVirtualization.IsEnabled = true;
            ...
            try
            {
                ...
                Assert.True(PooledCount(target, typeof(TypeA_Item)) > 0,
                    "Re-attaching should post warmup again; the skip while detached must not have " +
                    "marked warmup as done.");
            }
            finally
            {
                ContainerVirtualization.IsEnabled = true;
        ...
        /// <summary>
        /// A rooted panel with <see cref="VirtualizingStackPanel.EnableWarmup"/> set *before* it is
        /// attached, so the attach posts warmup to the dispatcher instead of the test calling
        /// <see cref="VirtualizingStackPanel.PerformWarmup"/> itself. MinPoolSizePerKey is well
        /// above the ~10 containers a 100px viewport realizes, so there is always a shortfall for
        /// warmup to fill and an empty pool means it did not run.
        /// </summary>
        private static (VirtualizingStackPanel, ItemsControl, TestRoot) CreateWarmupTarget(
            Orientation orientation)
        {
            var items = new ObservableCollection<object>(
                Enumerable.Range(0, 50).Select<int, object>(i => new TypeA_Item { Name = $"A{i}" }));
            ...
            var (target, _, itemsControl) = CreateUnrootedTarget<ItemsControl, VirtualizingStackPanel>(
                items: items,
                itemTemplate: template,
                orientation: orientation);
            target.EnableWarmup = true;
            ...
            return (target, itemsControl, root);
        }

        private static int PooledCount(VirtualizingStackPanel target, object recycleKey) =>
            target.RecyclePoolForTesting?.TryGetValue(recycleKey, out var pool) == true ? pool.Count : 0;

        /// <summary>
        /// Warmup depth must come from the template only for keys that template handed out.
        /// <see cref="FuncDataTemplate"/> implements <c>IVirtualizingDataTemplate</c> inertly — it
        /// backs every code-defined template in the framework — so resolving the depth from the
        /// template's *type* would quietly move every non-opted-in list off
        /// <c>DefaultWarmupPoolSizePerKey</c> onto whatever that type's default happens to be. Same
        /// shape as the bug where a plain XAML template capped the pool at 5.
        /// </summary>
        [Theory]
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Warmup_Depth_For_A_Template_That_Did_Not_Opt_In_Is_The_Panel_Default(Orientation orientation)
        {
            using var app = App();

            var items = new ObservableCollection<object>(
                Enumerable.Range(0, 30).Select<int, object>(i => new TypeA_Item { Name = $"A{i}" }));

            // A plain code-defined template: it can hand out keys, but this one does not.
            var template = new FuncDataTemplate<object>((_, _) => CanvasU(orientation, 10))
            {
                MinPoolSizePerKey = 1,
            };
            ...
            var (target, _, _) = CreateTarget(
                items: items,
                itemTemplate: new Optional<IDataTemplate?>(template),
                orientation: orientation);
            ...
            // 3 is DefaultWarmupPoolSizePerKey; the template's 1 must not be consulted.
            Assert.All(keys, entry => Assert.Equal(3, entry.Value));
        }

        [Theory]
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void DiscoverTemplateKeys_Finds_Multiple_Types(Orientation orientation)
        {
            using var app = App();
            ContainerVirtualization.IsEnabled = true;
        ...
        {
            // The panel has one view of "how big is element i" (GetElementSizeU, which applies the
            // element-size seam). Recording and re-checking sizes must both go through it. If
        ...
        //
        // Every shape runs in both orientations. The estimate reads the per-item size record and
        // the record stores a size along the scrolling axis without knowing which axis that is, so
        // a shape that destabilises the estimate has to do so identically for a horizontal panel;
        // asserting it only vertically would leave half of the claim untested.

        // Asserts that the visible realized elements tile without gaps or overlaps.
        private static void AssertRealizedContiguous(
            VirtualizingStackPanel target, string context, Orientation orientation = Orientation.Vertical)
        {
            var realized = target.GetRealizedElements()
                .Where(e => e is { IsVisible: true })
                .OrderBy(e => BoundsStartU(orientation, e!))
                .ToList();
            ...
            for (int i = 1; i < realized.Count; i++)
            {
                var prev = realized[i - 1]!;
                var curr = realized[i]!;
                var expectedStart = BoundsStartU(orientation, prev) + BoundsSizeU(orientation, prev);
                var actualStart = BoundsStartU(orientation, curr);
                Assert.True(
                    Math.Abs(actualStart - expectedStart) < 1,
                    $"Gap/overlap ({context}): item ends at {expectedStart}, next starts at {actualStart}");
            }
        }

        // Re-measures the panel `passes` times at the current offset and returns the
        // spread (max - min) of the reported extent along the scrolling axis. A stable estimate
        // keeps this small; an oscillating estimate produces a large spread.
        private static (double spread, double[] extents) MeasureExtentSpreadOverPasses(
            VirtualizingStackPanel target, ScrollViewer scroll, int passes, Orientation orientation)
        {
            ...
            for (int i = 0; i < passes; i++)
            {
                target.InvalidateMeasure();
                Layout(target);
                extents[i] = ExtentU(orientation, scroll);
            }
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Adversarial_Uniform_Extent_Is_Stable_Across_Repeated_Passes(Orientation orientation)
        {
            using var app = App();
            var items = CreateItemsU(orientation, 1000, _ => 30);
            var (target, scroll, _) = CreateTarget(
                items: items, itemTemplate: ItemTemplateU(orientation), orientation: orientation);

            scroll.Offset = OffsetU(orientation, 3000);
            Layout(target);

            var (spread, extents) = MeasureExtentSpreadOverPasses(target, scroll, 10, orientation);
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Adversarial_Uniform_Realized_Items_Stay_Contiguous(Orientation orientation)
        {
            using var app = App();
            var items = CreateItemsU(orientation, 1000, _ => 30);
            var (target, scroll, _) = CreateTarget(
            ...
            for (double offset = 0; offset < 3000; offset += 25)
            {
                scroll.Offset = OffsetU(orientation, offset);
                Layout(target);
                AssertRealizedContiguous(target, $"uniform @ {offset}", orientation);
            }
        }
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Adversarial_Bimodal_Extent_Is_Stable_Across_Repeated_Passes(Orientation orientation)
        {
            using var app = App();
            var items = CreateItemsU(orientation, 1000, i => i % 2 == 0 ? 40 : 300);
            var (target, scroll, _) = CreateTarget(
                items: items, itemTemplate: ItemTemplateU(orientation), orientation: orientation);

            scroll.Offset = OffsetU(orientation, 5000);
            Layout(target);
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Adversarial_Bimodal_Realized_Items_Stay_Contiguous(Orientation orientation)
        {
            using var app = App();
            var items = CreateItemsU(orientation, 1000, i => i % 2 == 0 ? 40 : 300);
            var (target, scroll, _) = CreateTarget(
            ...
            for (double offset = 0; offset < 5000; offset += 40)
            {
                scroll.Offset = OffsetU(orientation, offset);
                Layout(target);
                AssertRealizedContiguous(target, $"bimodal @ {offset}", orientation);
            }
        }
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Adversarial_Outliers_Extent_Is_Stable_Across_Repeated_Passes(Orientation orientation)
        {
            using var app = App();
            var items = CreateItemsU(orientation, 1000, i => i % 50 == 0 ? 2000 : 20);
            var (target, scroll, _) = CreateTarget(
                items: items, itemTemplate: ItemTemplateU(orientation), orientation: orientation);
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Adversarial_Outliers_Realized_Items_Stay_Contiguous(Orientation orientation)
        {
            using var app = App();
            var items = CreateItemsU(orientation, 1000, i => i % 50 == 0 ? 2000 : 20);
            var (target, scroll, _) = CreateTarget(
            ...
            for (double offset = 0; offset < 6000; offset += 37)
            {
                scroll.Offset = OffsetU(orientation, offset);
                Layout(target);
                AssertRealizedContiguous(target, $"outliers @ {offset}", orientation);
            }
        }
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Adversarial_MonotonicRamp_Extent_Is_Stable_Across_Repeated_Passes(Orientation orientation)
        {
            using var app = App();
            var items = CreateItemsU(orientation, 1000, i => 10 + i * 2);
            var (target, scroll, _) = CreateTarget(
                items: items, itemTemplate: ItemTemplateU(orientation), orientation: orientation);

            scroll.Offset = OffsetU(orientation, 10000);
            Layout(target);
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Adversarial_MonotonicRamp_Realized_Items_Stay_Contiguous(Orientation orientation)
        {
            using var app = App();
            var items = CreateItemsU(orientation, 1000, i => 10 + i * 2);
            var (target, scroll, _) = CreateTarget(
            ...
            for (double offset = 0; offset < 10000; offset += 100)
            {
                scroll.Offset = OffsetU(orientation, offset);
                Layout(target);
                AssertRealizedContiguous(target, $"ramp @ {offset}", orientation);
            }
        }
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Adversarial_AsyncGrow_Extent_Does_Not_Oscillate_Across_Repeated_Passes(Orientation orientation)
        {
            using var app = App();
            var items = CreateItemsU(orientation, 1000, _ => 84);
            var (target, scroll, _) =
                CreateTarget<ItemsControl, VirtualizingStackPanelAsyncGrow>(
                    items: items, itemTemplate: ItemTemplateU(orientation), orientation: orientation);

            scroll.Offset = OffsetU(orientation, 2000);
            Layout(target);
            ...
            var (spread, extents) = MeasureExtentSpreadOverPasses(target, scroll, 12, orientation);
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Adversarial_AsyncGrow_Realized_Items_Stay_Contiguous(Orientation orientation)
        {
            using var app = App();
            var items = CreateItemsU(orientation, 1000, _ => 84);
            var (target, scroll, _) =
            ...
            for (int pass = 0; pass < 12; pass++)
            {
                target.InvalidateMeasure();
                Layout(target);
                AssertRealizedContiguous(target, $"async-grow pass {pass}", orientation);
            }
        }
            ...
            Orientation orientation)
        {
            ...
            // The leading visible element is the scroll anchor. Its index and screen
            // position must not jump as off-anchor items after it flip to their loaded size.
            var anchor = target.GetRealizedElements()
                .Where(e => e is { IsVisible: true })
                .OrderBy(e => BoundsStartU(orientation, e!))
                .First()!;
            var anchorIndex = itemsControl.IndexFromContainer((Control)anchor);
            var anchorStartU = BoundsStartU(orientation, anchor);
            var offsetU = ScrollOffsetU(orientation, scroll);
            ...
            for (int pass = 0; pass < 12; pass++)
            {
                ...
                Assert.True(Math.Abs(ScrollOffsetU(orientation, scroll) - offsetU) < 1.0,
                    $"Scroll offset jumped on pass {pass}: {offsetU} -> {ScrollOffsetU(orientation, scroll)}");
                ...
                {
                    Assert.True(Math.Abs(BoundsStartU(orientation, stillRealized) - anchorStartU) < 1.0,
                        $"Anchor item {anchorIndex} jumped on pass {pass}: {anchorStartU} -> " +
                        $"{BoundsStartU(orientation, stillRealized)}");
                }
            }
        }
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Adversarial_CrossRegion_Extent_Is_Window_Independent(Orientation orientation)
        {
            using var app = App();
            var items = CreateItemsU(orientation, 200, i => i < 100 ? 40 : 300);
            var (target, scroll, _) = CreateTarget(
                items: items, itemTemplate: ItemTemplateU(orientation), orientation: orientation);
            ...
            {
                scroll.Offset = OffsetU(orientation, o);
                Layout(target);
            }
            ...
            for (int pass = 0; pass < 2; pass++)
            {
                foreach (var o in offsets)
                {
                    scroll.Offset = OffsetU(orientation, o);
                    Layout(target);
                    extents.Add(ExtentU(orientation, scroll));
                }
            }
            ...
            // ... and it settled on that true total rather than on some other stable number. A
            // stable-but-wrong extent is what this test would otherwise accept: if the priming
            // sweep never moved the window (the shape of an axis mix-up, where the offsets are
            // applied to the axis the panel is not scrolling), every pass would report the same
            // head-only estimate and the spread check alone would pass.
            Assert.All(extents, e => Assert.InRange(e, 34000 - 1, 34000 + 1));
        }
        ...
        // libraries emit when a batch of edits crosses their reset threshold — and each of those
        // in both orientations, because the keep / reuse / recycle decisions are taken in (u, v)
        // space and a horizontal panel runs exactly the same code with the axes swapped.
        ...
        /// <summary>Every edit kind, at every position relative to the window, in both orientations.</summary>
        public static IEnumerable<object[]> CollectionEditCases() =>
            from edit in Enum.GetValues<CollectionEdit>()
            from position in Enum.GetValues<EditPosition>()
            from orientation in BothOrientations
            select new object[] { edit, position, orientation };

        [Theory]
        [MemberData(nameof(CollectionEditCases))]
        public void Collection_Edit_Keeps_Every_Container_On_Its_Own_Item(
            CollectionEdit edit, EditPosition position, Orientation orientation)
        {
            using var app = App();

            // Varied sizes so a mis-mapping shows up as a position error too, not just a
            // wrong DataContext.
            var items = new ObservableCollection<object>(
                CreateItemsU(orientation, 200, i => 10 + (i % 4) * 20));
            ...
            switch (edit)
            {
                case CollectionEdit.InsertOne:
                    items.Insert(at, CreateItemU(orientation, 900, 55));
                    break;
                case CollectionEdit.InsertRange:
                    for (var i = 0; i < 3; i++)
                        items.Insert(at + i, CreateItemU(orientation, 900 + i, 35 + i * 10));
                    break;
                ...
                case CollectionEdit.Replace:
                    items[at] = CreateItemU(orientation, 900, 65);
                    break;
            ...
            var context = $"{edit} at {position} (index {at}, {orientation}), window was [{first}..{last}]";
            AssertContainerIndexMappingIsCorrect(target, itemsControl, items, context);
            AssertRealizedContiguous(target, context, orientation);
        }

        [Theory]
        [InlineData(EditPosition.BeforeWindow, Orientation.Vertical)]
        [InlineData(EditPosition.EarlyInWindow, Orientation.Vertical)]
        [InlineData(EditPosition.LateInWindow, Orientation.Vertical)]
        [InlineData(EditPosition.AfterWindow, Orientation.Vertical)]
        [InlineData(EditPosition.BeforeWindow, Orientation.Horizontal)]
        [InlineData(EditPosition.EarlyInWindow, Orientation.Horizontal)]
        [InlineData(EditPosition.LateInWindow, Orientation.Horizontal)]
        [InlineData(EditPosition.AfterWindow, Orientation.Horizontal)]
        public void Insert_Coalesced_Into_A_Reset_Keeps_Every_Container_On_Its_Own_Item(
            EditPosition position, Orientation orientation)
        {
            ...
            // Uniform sizes on purpose: the inserted item is the same size as everything else,
            ...
            // sizes would let a size mismatch trip re-realization and mask the bug.
            var backing = CreateItemsU(orientation, 400, _ => 10);
            ...
            mutated.Insert(at, CreateItemU(orientation, 900, 10));
            items.Reset(mutated);
            Layout(target);

            var context = $"insert at {position} (index {at}, {orientation}) coalesced into a " +
                          $"Reset, window was [{first}..{last}]";
            AssertContainerIndexMappingIsCorrect(target, itemsControl, items, context);
            AssertRealizedContiguous(target, context, orientation);
        }
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Reorder_Coalesced_Into_A_Reset_Keeps_Every_Container_On_Its_Own_Item(Orientation orientation)
        {
            // A sort reported as a Reset: every item survives but almost none keeps its index.
            using var app = App();

            var backing = CreateItemsU(orientation, 200, i => 10 + (i % 4) * 20);
            var items = new ResettingObservableCollection<object>(backing);
            ...
            var context = $"reversed via Reset ({orientation})";
            AssertContainerIndexMappingIsCorrect(target, itemsControl, items, context);
            AssertRealizedContiguous(target, context, orientation);
        }
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Append_Coalesced_Into_A_Reset_Keeps_Every_Container_On_Its_Own_Item(Orientation orientation)
        {
            ...
            appended.AddRange(Enumerable.Range(200, 50)
                .Select(i => CreateItemU(orientation, i, 10 + (i % 4) * 20)));
            items.Reset(appended);
            Layout(target);

            var context = $"append via Reset ({orientation})";
            AssertContainerIndexMappingIsCorrect(target, itemsControl, items, context);
            AssertRealizedContiguous(target, context, orientation);
        ...
        //
        // Both orientations: the compensation is `StartU -= preDelta` over sizes along the
        // scrolling axis, so it is the same arithmetic for a horizontal panel and a threshold
        // sneaking back in on one axis only would otherwise go unnoticed.

        [Theory]
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Sub_Pixel_Pre_Anchor_Resize_Is_Compensated_Not_Absorbed(Orientation orientation)
        {
            ...
            var items = CreateItemsU(orientation, 50, _ => 50);
            ...
            var (target, scroll, itemsControl) = CreateTarget(
                items: items,
                itemTemplate: UnroundedItemTemplateU(orientation),
                styles: new[]
                {
            ...
            scroll.Offset = OffsetU(orientation, 600);
            Layout(target);

            var firstRealized = target.FirstRealizedIndex;
            var anchorIdx = (int)(ScrollOffsetU(orientation, scroll) / 50);
            ...
            ((ISizedItemU)items[growIdx]).SizeU = 50.5;
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Second_Resize_In_Same_Arrange_Cycle_Is_Still_Compensated(Orientation orientation)
        {
            ...
            // before the anchor by 20px each time (50 -> 70 -> 90) — content settling in steps.
            // Both steps must be compensated: total StartU shift 40px.
            var growIdx = firstRealized + 1;
            var containerConstraint = orientation == Orientation.Horizontal
                ? new Size(double.PositiveInfinity, 100)
                : new Size(100, double.PositiveInfinity);

            foreach (var sizeU in new double[] { 70, 90 })
            {
                var container = itemsControl.ContainerFromIndex(growIdx) as Control;
                Assert.NotNull(container);
                ((ISizedItemU)items[growIdx]).SizeU = sizeU;
                ...
                container.Measure(containerConstraint);
                Assert.Equal(
                    sizeU,
                    orientation == Orientation.Horizontal
        ...
        [InlineData(Orientation.Vertical)]
        [InlineData(Orientation.Horizontal)]
        public void Anchor_Stays_Put_Across_Repeated_Passes_With_Few_Realized_Items(Orientation orientation)
        {
            ...
            var items = CreateItemsU(orientation, 500, i => i < 5 ? 5 : 300);
            ...
            for (var pass = 0; pass < 10; pass++)
            {
                ...
                Assert.True(Math.Abs(BoundsStartU(orientation, still!) - anchorStartU) < 1.0,
                    $"Anchor item {anchorIndex} drifted on pass {pass}: {anchorStartU} -> " +
                    $"{BoundsStartU(orientation, still!)}. Anchor compensation is being skipped.");
            }
        }
            ...
            Orientation orientation)
        {
            ...
            foreach (var o in offsets)
            {
                scroll.Offset = OffsetU(orientation, o);
                Layout(target);
                first.Add(ExtentU(orientation, scroll));
            }
            ...
            foreach (var o in offsets)
            {
                scroll.Offset = OffsetU(orientation, o);
                Layout(target);
                second.Add(ExtentU(orientation, scroll));
            }
