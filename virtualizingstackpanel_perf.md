...
### 11. AdjustElementSize test hook

**Problem:** Many of the virtualizing panel's features (cycle breaker, frozen extent, ValidateStartU) are designed to handle non-deterministic measurement — controls whose `DesiredSize` varies across passes (async image loading, text wrapping, deferred bindings). In the deterministic test environment, these code paths are never triggered because item measurements are stable.

**Fix:** Added `protected internal virtual double AdjustElementSize(int index, double measuredSizeU)` to `VirtualizingStackPanel`. This is called after each element is measured during realization, in both the forward and backward loops. The default implementation returns the measured size unchanged. Test subclasses override it to inject measurement instability (oscillating sizes, gradual drift) that simulates real-world non-deterministic behavior.

### 12. ControlCatalog: ComplexVirtualizationPage image support
...
- Frozen extent bottom capping when viewport overshoots past all content (fast scroll)
- Frozen extent grows immediately when actual extent exceeds frozen value (undershoot fix)
