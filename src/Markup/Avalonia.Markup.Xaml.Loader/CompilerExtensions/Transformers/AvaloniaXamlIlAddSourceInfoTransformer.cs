...
{
    /// <summary>
    /// An XAMLIL AST transformer that injects <see cref="SourceInfo"/> metadata into the generated XAML code.
    /// </summary>
    /// <remarks>
    /// This transformer runs during XAML compilation and attaches <see cref="Source.SourceInfoProperty"/> 
    ...
    /// <para/>
    /// Adding <see cref="SourceInfo"/> helps tooling like the Avalonia designer or visual inspectors 
        ...
        public IXamlAstNode Transform(AstTransformationContext context, IXamlAstNode node)
        {
            if (CreateSourceInfo && node is XamlAstObjectNode objNode)
            {
                ...
                if (avaloniaTypes.Control.IsAssignableFrom(clrType))
                {
                    ...
                    {
                        valueNode = new XamlAstNewClrObjectNode(node, sourceInfoTypeRef, avaloniaTypes.SourceInfoConstructorFull, new List<IXamlAstValueNode>
                        {
                    ...
                    {
                        valueNode = new XamlAstNewClrObjectNode(node, sourceInfoTypeRef, avaloniaTypes.SourceInfoConstructor, new List<IXamlAstValueNode>
                        {
                            new XamlConstantNode(node, avaloniaTypes.XamlIlTypes.Int32, line),
                            new XamlConstantNode(node, avaloniaTypes.XamlIlTypes.Int32, col)
                        });
