...
{
    internal class DirectCompositedWindowSurface : IDirect3D11TexturePlatformSurface, IDisposable, IBlurHost
    {
        ...
        {
            _shared.EnsureCompositionDevice(d3dDevice);

            _window ??= new DirectCompositedWindow(_info, _shared);
            _window.SetBlur(_blurEffect);

            return new DirectCompositedWindowRenderTarget(context, _shared, _window);
        }
    ...
    internal class DirectCompositedWindowRenderTarget : IDirect3D11TextureRenderTarget
    {
        ...
        public DirectCompositedWindowRenderTarget(
            IPlatformGraphicsContext context, DirectCompositionShared shared, DirectCompositedWindow window)
        {
            _context = context;
            _window = window;
            ...
            try
            {
                _surface = shared.Device?.CreateVirtualSurface(1, 1, DXGI_FORMAT.DXGI_FORMAT_B8G8R8A8_UNORM,
                    DXGI_ALPHA_MODE.DXGI_ALPHA_MODE_PREMULTIPLIED) ?? throw new InvalidOperationException("IDCompositionDevice was not yet created.");
            }
            catch
            {
                _surface?.Dispose();
