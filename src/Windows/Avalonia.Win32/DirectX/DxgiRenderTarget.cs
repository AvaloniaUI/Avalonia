...
namespace Avalonia.Win32.DirectX
{
    internal unsafe class DxgiRenderTarget : EglPlatformSurfaceRenderTargetBase
    {
        ...
        public DxgiRenderTarget(EglGlPlatformSurface.IEglWindowGlPlatformSurfaceInfo window, EglContext context,
            DxgiConnection connection) : base(context)
        {
            _window = window;
        ...
        private void CreateSurface(PixelSize expectedPixelSize, bool isTransparency)
        {
            _swapChain?.Dispose();
            _swapChain = null;
        ...
        public override IGlPlatformSurfaceRenderingSession BeginDrawCore(IRenderTarget.RenderTargetSceneInfo sceneInfo)
        {
            ...
            var shouldTransparency = sceneInfo.TransparencyLevel != CompositionTransparencyLevel.None &&
                                     DxgiConnection.IsTransparencySupported();

            if (_swapChain is null)
            {
                CreateSurface(size, shouldTransparency);
            }
