...
class WSurface : IPersistentWaylandObject, IWSurface
{
    ...
    internal void RegisterRenderTarget(IDisposable renderTarget) => _activeRenderTargets.Add(renderTarget);
    internal void UnregisterRenderTarget(IDisposable renderTarget) => _activeRenderTargets.Remove(renderTarget);
...
class WXdgTopLevel : WXdgShellSurface, IWXdgTopLevel
{
    ...
    private class DecorationListener(WXdgTopLevel p) : ZxdgToplevelDecorationV1.Listener
    {
        protected override void Configure(ZxdgToplevelDecorationV1 eventSender, ZxdgToplevelDecorationV1.ModeEnum mode)
        {
            ...
            if (p._basicInitCompletedTcs.Task.IsCompleted)
