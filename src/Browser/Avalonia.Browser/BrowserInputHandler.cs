...
internal class BrowserInputHandler
{
    ...
    /// <summary>
    /// Timestamp for events that do not come through the input ring (composition text, drag and drop).
    /// Ring events carry the DOM event timestamp instead.
    /// </summary>
    ...
    // ---- Ring-buffer events (decoded by BrowserInputQueue) ----
    ...
    // ---- Direct-path events (still one interop call each; JS flushes the ring first) ----
