 ...
 *
 * The ring lives in WASM linear memory and is allocated by C# (BrowserInputQueue.cs), which owns the
 * control block and the consumer index. JS appends encoded records and owns the producer index and a
 * spill list that is used only when the ring is full. Layouts below must match BrowserInputQueue.cs.
 *
...
export class InputQueue {
     ...
     * threadingEnabled means the runtime's managed code does not run on this thread, so only
     * asynchronous (Task-returning) exports may be called from here.
     */
    ...
    /**
     * Posts a key event and, when the runtime allows it, flushes synchronously.
     * Returns Avalonia's handled state, or false when it cannot be known on this runtime.
     */
     ...
     * Direct-path events (composition, drag and drop) call this before their interop call.
     */
    public static flush(): void {
        ...
        if (this.threadingEnabled) {
            // Cannot call synchronously; an asynchronous wake is queued ahead of the caller's own
            // asynchronous call, and the runtime processes them in order.
            if (!this.isEmpty()) {
     ...
     * Returns the number of records moved; 0 means the first spilled record does not fit an empty ring.
     */
    ...
    /** Appends an encoded record; returns false when there is no room. */
    ...
    private static writePadding(offset: number, size: number): void {
        // The consumer only reads type and size of a padding record, so 8 bytes are enough; the
        // remaining bytes of the padded area are skipped without being read. `size` is always a
        // multiple of 8, so the header never runs past the end of the data area.
    ...
    private static scheduleWake(): void {
        ...
        // A macrotask, not a microtask: microtasks would run before the DOM handler's task ends and
        // re-create the per-event entry into C#.
        this.wakeChannel.port2.postMessage(null);
    }

    private static onWake(): void {
        // A synchronous flush may already have emptied the ring; C# treats that as a no-op.
        if (this.threadingEnabled) {
            JsExports.InputHelper.OnInputWakeAsync();
        } else {
    ...
    private static flushSync(): boolean {
        if (this.threadingEnabled) {
            // The main thread cannot wait for the managed thread; the handled state is never known here.
            return false;
        }
