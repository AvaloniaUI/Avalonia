...
namespace Avalonia.Media
{
    ...
    readonly struct HsvColor : IEquatable<HsvColor>
    {
        ...
        {
#if !BUILDTASK
            return TryParseSpan(s.AsSpan(), out hsvColor);
#else
            return TryParseString(s, out hsvColor);
#endif
        }
        ...
        private static bool TryParseSpan(ReadOnlySpan<char> s, out HsvColor hsvColor)
        {
            ...
            if (span.Length >= 11 &&
                span.StartsWith("hsva(".AsSpan(), StringComparison.OrdinalIgnoreCase) &&
                span[span.Length - 1] == ')')
            {
                ...
                hasAlpha = true;
                     ...
                     span.StartsWith("hsv(".AsSpan(), StringComparison.OrdinalIgnoreCase) &&
                     span[span.Length - 1] == ')')
            {
            ...
            if (hasAlpha)
            {
                ...
                if (comp4.Length == 0)
                {
                    // hsva() with only 3 components - treat as hsv
                    if (comp1.Trim().TryParseDouble(NumberStyles.Number, CultureInfo.InvariantCulture, out double hue) &&
                        ...
                        TryParsePercentageOrDouble(comp3.Trim(), out double value))
                    {
        ...
        private static bool TryParseString(string? s, out HsvColor hsvColor)
        {
