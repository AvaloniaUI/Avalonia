...
namespace Avalonia.Media.TextFormatting.Unicode
{
    ...
    /// navigation: grapheme-cluster (UAX-29) character stepping, UAX-29 word boundaries, UAX-29
    /// sentence boundaries, logical-line bounds, move-by-unit, and contiguous-change diffing.
    /// Everything operates on a <see cref="ReadOnlySpan{T}"/> and code-unit offsets so both
    /// navigators use one implementation; a <see cref="string"/> caller converts implicitly.
    /// </summary>
    internal static class TextSegmentation
    {
        ...
        public static (int Start, int End) WordBounds(int offset, ReadOnlySpan<char> text)
        {
            ...
            {
                var end = segment.Offset + segment.Text.Length;
                if (clamped < end)
                {
        ...
        // UAX-29 sentence segmentation (rules SB1–SB11, SB998) via SentenceBreakEnumerator.
        // Enumerates from the document start to find the segment containing the offset; O(offset),
        // matching the word-break path.
        public static (int Start, int End) SentenceBounds(int offset, ReadOnlySpan<char> text)
        {
            ...
            while (enumerator.MoveNext(out var segment))
            {
                var segEnd = segment.Offset + segment.Text.Length;
                lastStart = segment.Offset;
                lastEnd = segEnd;
                if (clamped < segEnd)
                {
                    return (segment.Offset, segEnd);
                }
            }

            // offset == text.Length: return the last segment.
