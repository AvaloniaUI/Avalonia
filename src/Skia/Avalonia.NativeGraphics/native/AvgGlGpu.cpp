...
HRESULT AvgGlGpu::Create(bool gles, IAvgGetProcAddressDelegate *getProcAddressDelegate, IAvgGpu **pGpu) {
    *pGpu = nullptr;
    auto iface = gles
        ? GrGLMakeAssembledGLESInterface(getProcAddressDelegate, (GrGLGetProc)GlGetProc)
...
HRESULT AvgGlRenderTarget::CreateDrawingContext(IAvgDrawingContext **ppv)
{
    ...
    GrBackendRenderTarget backendRenderTarget(size.Width, size.Height, session->GetSampleCount(), session->GetStencilSize(),
                                              {
                                                      (uint)session->GetFboId(),32856
                                              });
    auto surface = SkSurface::MakeFromBackendRenderTarget(_gpu->getGrContext().get(), backendRenderTarget,
                                           session->GetIsYFlipped() ? kTopLeft_GrSurfaceOrigin : kBottomLeft_GrSurfaceOrigin,
                                           SkColorType::kRGBA_8888_SkColorType, nullptr, nullptr);
    *ppv = new AvgDrawingContext(surface, surface->getCanvas(), session);
    return 0;
}
