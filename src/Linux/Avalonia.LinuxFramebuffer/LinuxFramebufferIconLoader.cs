...
internal class LinuxFramebufferIconLoader : IPlatformIconLoader
{
    private unsafe class FBIconData : IWindowIconImpl, IFramebufferPlatformSurface
    {
        private readonly int _width;
        private readonly int _height;
        private readonly uint[] _bdata;
        private UIntPtr[] Data { get; }

        public FBIconData(IBitmap bitmap)
        {
    ...
    private IWindowIconImpl LoadIcon(IBitmap bitmap)
    {
        var rv = new FBIconData(bitmap);
        bitmap.Dispose();
        return rv;
    }
    public IWindowIconImpl LoadIcon(string fileName) => 
        LoadIcon(new Bitmap(fileName));

    public IWindowIconImpl LoadIcon(Stream stream) => 
        LoadIcon(new Bitmap(stream));
