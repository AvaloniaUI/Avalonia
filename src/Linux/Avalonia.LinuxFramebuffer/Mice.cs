...
namespace Avalonia.LinuxFramebuffer
{
    public unsafe class Mice
    {
        ...
        private void Worker()
        {
            var mouseDevices = EvDevDevice.InputDevices;
        ...
        {
            Console.WriteLine($"# ProcessEvent(device: [Name:{device.Name}, Type:{device.Type}], ev: [type:{(EvType)ev.type}, value:{ev.value}, code:{ev.code}])");

            switch (device.Type)
            {
                case EvDevDeviceType.Mouse:
                    {
                    ...
                    break;
                case EvDevDeviceType.Touch:
                    {
                        switch ((EvType)ev.type)
                        {
                            case EvType.EV_ABS:
                                {
                                    switch ((AbsAxis)ev.code)
                                    {
                                        case AbsAxis.ABS_MT_POSITION_X:
                                            {
                                                _x = ev.value;
                                            }
                                            break;
                                        case AbsAxis.ABS_MT_POSITION_Y:
                                            {
                                                _y = ev.value;
                                            }
                                            break;
                                    ...
                                    Event?.Invoke(new RawMouseEventArgs(LinuxFramebufferPlatform.MouseDevice, 
                                        LinuxFramebufferPlatform.Timestamp,
                                        LinuxFramebufferPlatform.TopLevel.InputRoot, 
                            ...
                            case EvType.EV_KEY:
                                {
                                    RawMouseEventType? type = null;
                                    switch ((EvKey)ev.code)
                                    {
                                        case EvKey.BTN_TOUCH:
                                            type = ev.value == 0 ? RawMouseEventType.LeftButtonUp : RawMouseEventType.LeftButtonDown;
                                            break;
                                    }
