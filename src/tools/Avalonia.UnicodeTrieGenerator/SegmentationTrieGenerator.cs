...
///   bit     23: Emoji                  (flag)
///   bit     24: Emoji_Presentation     (flag)
///   bit     25: Default_Ignorable_Code_Point (flag)
...
internal static class SegmentationTrieGenerator
{
    ...
    public static UnicodeTrie Execute(string outputDir, out Dictionary<int, uint> values)
    {
        ...
        var emojiData = ReadBreakData("emoji/emoji-data.txt");

        foreach (var (start, end, breakType) in emojiData)
        {
            if (!graphemeMappings.TryGetValue(breakType.Replace("_", ""), out var idx))
                continue;
            AddRange(values, start, end, (uint)idx,
        ...
        // ── Single-bit properties ─────────────────────────────────────────────────
        // Emoji and Emoji_Presentation ride along in the emoji data already read for
        // Extended_Pictographic; Default_Ignorable_Code_Point comes from the same
        // DerivedCoreProperties.txt as the Indic conjunct break data.
        foreach (var (start, end, property) in emojiData)
        {
            ...
            if (flag != 0)
                SetFlag(values, start, end, flag, initialValue);
        }

        foreach (var (start, end) in ReadDerivedCoreProperty("Default_Ignorable_Code_Point"))
        {
            SetFlag(values, start, end, UnicodeData.DEFAULTIGNORABLE_FLAG, initialValue);
        }
    ...
    private static void SetFlag(Dictionary<int, uint> values, int start, int end, uint flag, uint defaultValue)
    {
        for (var codepoint = start; codepoint <= end; codepoint++)
        {
            if (!values.TryGetValue(codepoint, out var existing))
