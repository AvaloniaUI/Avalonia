...
namespace ControlCatalog.ViewModels
{
    public class ComplexVirtualizationPageViewModel : ViewModelBase
    {
        ...
        public ComplexVirtualizationPageViewModel()
        {
            ...
            // Pre-load sample images for PhotoItems
            var cachedImages = new Bitmap[SampleImages.Length];
            for (int j = 0; j < SampleImages.Length; j++)
            {
                try
                {
                    var uri = new Uri(SampleImages[j]);
                    cachedImages[j] = new Bitmap(AssetLoader.Open(uri));
                }
                catch
                {
            ...
            for (int i = 0; i < 5000; i++)
            {
                ...
                switch (type)
                {
                    ...
                    case 3:
                        ...
                        var imageIndex = random.Next(SampleImages.Length);
                        // Varying image heights to stress heterogeneous virtualization
                        var imageHeight = random.Next(5) switch
                        {
                            0 => 60,    // small thumbnail
                            1 => 100,   // medium
                            2 => 150,   // large
                            3 => 200,   // extra large
                            _ => 250    // hero image
